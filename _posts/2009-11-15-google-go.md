...
- The nineteen seventies architecture for compiler and run-time. As I outlined in <a href="http://www.jillesvangurp.com/2009/10/24/maven-the-way-forward/">my last post</a>, I believe compilation is fundamentally a run-time optimization these days. With Go, the compilation work is allegedly light to begin with due to the wise decision to keep the language simple. So why bother at all with compiling and why not just leave things to the run-time to figure out when you actually run the code? Integrate the compiler into the run-time and remove compilation as a concern for developers.
