...
date: 2016-02-10T19:57:40+00:00
author: Jilles
layout: revision
guid: http://www.jillesvangurp.com/2014/12/06/1697-autosave-v1/
permalink: /2016/02/10/1697-autosave-v1/
---
** Update 2016-02-10 ** I recently released our Elasticsearch client code on <a href="https://github.com/Inbot/inbot-es-http-client">github</a>. It includes the stategy I outline below. Simply create a dao using the CrudOperationsFactory, be sure to enable redis caching there, and use the modifiedIds() on the dao to retrieve a list of recently modified ids.

Elasticsearch promises real-time search and nearly delivers on this promise. The problem with 'nearly; is that in interactive systems, it is actually unacceptable to not have user changes reflect in the any query results. <a href="http://en.wikipedia.org/wiki/Eventual_consistency">Eventual consistency</a> is nice but it also means occasionally being inconsistent, which is not so nice for users, or worse, product managers, who typically don't understand these things and report them as bugs. At Inbot, this aspect of using Elasticsearch has been keeping us busy. It would be awfully convenient if it never returned stale data.
...
We started addressing this with a few client side hacks like simply replacing list results with what we just edited via the API, updating local caches, etc. Writing such code is error prone and tedious. So we came up with a better solution: use <a href="http://redis.io">Redis</a>. The same DAO I described in <a href="http://www.jillesvangurp.com/2014/12/03/optimistic-locking-for-updates-in-elasticsearch/">my recent article on optimistic locking with elasticsearch</a> also stores the id of any modified documents in a shortlived data structure in Redis. Redis provides in memory data structures such as lists, sets, and hash maps and comes with a ton of options. The nice thing about Redis is that it scales quite well for small things and has a very low latency API. So, it is quite cheap to use it for things like caching.
